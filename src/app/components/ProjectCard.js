...
export default function ProjectCard({
  ...
  return (
    <div className="shadow-lg rounded-xl hover:shadow-xl hover:-translate-y-1 overflow-hidden bg-[#37353E]">
      <Image
        src={`${projectImage}`}
        width={1000}
        height={540}
      ...
      <div className="p-5 ">
        ...
        <div className="mt-2 text-center">
          <p className="text-base indent-5">{`${projectDescription}`}</p>
        </div>
        <div className="flex justify-center flex-wrap mt-2 gap-1">
          {tags.map((tag, index) => (
            <div
              key={index}
              className="px-3 my-auto py-1 rounded-full bg-black/80 text-gray-300 text-sm"
            >
              {tag}
            </div>
          ))}
